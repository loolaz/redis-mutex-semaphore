...
exports.factory = function(connect_options, cb){
	...
	Mutex.prototype._extend = function(muid, timeout){
		var that = this,
			...
			script = 'local muid = "' + muid + '" \
						if redis.call("GET",KEYS[1]) == muid then \
						  return redis.call("PEXPIRE",KEYS[1], ' + newTimeout + ') \
						else \
						  return nil \
						end';

		return new Promise(function(resolve, reject){
			that.client.eval(script, 1, that.key, function(err, result){
				if(err){
					reject(err);
				}
				else{
	...
	Mutex.prototype.extend = function(muid, timeout, cb){
		var p = this._extend(muid, timeout);
		return this._convertPromiseCall(p, cb);
	};
