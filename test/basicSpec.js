		...
		for(var i =0 ; i < 10 ; i++){
			...
			redisSemaphoreClient.waitingFor(100, function(err, result){
				if(result){
					...
					if(j === 10){
						console.log('2. Testing semaphore with callback has been done by ' + (Date.now() - s));
						done();
					}
					setTimeout(function(){
		...
		for(var i =0 ; i < 10 ; i++){
			...
			redisSemaphoreClient.waitingFor(100).then(function(result){
				if(result){
					...
					console.log('... accquire : (' + j + ')');
					if(j === 10){
						console.log('3. Testing semaphore with promise has been done by ' + (Date.now() - s));
						done();
					}					
				}
		...
		for(var i =0 ; i < 10 ; i++){
			...
			redisMutexClient.waitingFor(100, function(err, id){
				if(id){
					...
					if(j === 10){
						console.log('4. Testing mutex with callback has been done by ' + (Date.now() - s));
						done();
					}
					setTimeout(function(){
		...
		for(var i =0 ; i < 10 ; i++){
			...
			redisMutexClient.waitingFor(100).then(function(id){
				if(id){
					...
					console.log('... lock : (' + j + ') : ' + id);
					if(j === 10){
						console.log('5. Testing mutex with promise has been done by ' + (Date.now() - s));
						done();
					}					
